...
3. **The build-status row is updated** — an explicit exit criterion, not a by-product: slice built, Verified at `merge`, proof named, *Completes* column filled **as an explicit list of IDs** (never a phrase such as "all `ENTITY-*`"), because the fitness test for acceptance 3 parses it. A slice that changes the schema rule table regenerates `lspd.schema.json` and the README checksum line in the same commit, or gate 5 is red.
