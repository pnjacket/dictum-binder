...
> One-line: six headless slices (the last verification-only) from `main`, a Definition of Done that is Quality's seven gates plus a build-status row, an empty substitution set, an LLM-written `bindings.yaml` (written without the tool, errors fixed by hand, layout conformed with `dbind format`) right before the `v1.0.0` tag, and the in-repo record as the only tracker.
...
- **Branching inside the build is not governed by the doc set.** Until `v1.0.0` the work happens on `main`; how the implementer arranges local branches or commits between checkpoints is an implementation-level matter the operator deliberately leaves outside this document. `deferred` by decision — `[FUTURE-SCOPE]` re-entry: post-release branching is governed when a second release is planned.
...
- No release-infrastructure gate distinct from the merge gate beyond the checklist below: nothing is deployed. The substitution set is **empty and stated** in Operations' fidelity map (`ENV-LOCAL`, `ENV-CI`): no externals; nothing substituted; own code real at every gate. Operations is **in** on that map, its provisioning, and the `TOOL-*` register only (pulled in 2026-09-17 after the second audit: a toolchain to pin is an environment fact per the spec; the earlier "both halves evaluated" rationale had missed that clause).
...
3. **The build-status row is updated** — an explicit exit criterion, not a by-product: slice built, Verified at `merge`, proof named, *Completes* column filled **as an explicit list of IDs** (never a phrase such as "all `ENTITY-*`"), because the fitness test for acceptance 3 parses it. **All six rows' *Completes* lists are pre-filled at the record's creation in slice 1** (from the playbook), so the union check holds from day one; only Built / Verified / Proof change as slices land. A slice that changes the schema rule table regenerates `dbind.schema.json` and the README checksum line in the same commit, or gate 5 is red.
...
3. **Release slice.** An LLM writes `bindings.yaml` for this repository in canonical style without invoking `dbind`, mapping every `COMPONENT-*`, `CLI-*`, `ENTITY-*`, `INV-*` to `src/dbind/` symbols and tests. It then runs `dbind validate`, fixes any error-level findings **by hand** (writes are refused while they stand), runs `dbind format` to conform the layout, confirms `dbind validate` and `dbind format --check` both exit 0, and ticks the release-gate checklist in the record. `pyproject.toml` says `1.0.0`; `git tag v1.0.0`.
...
5. At the tag `v1.0.0`: `pyproject.toml` version equals `1.0.0`; a fresh-venv wheel install passes the E2E tier; README checksum equals `dbind schema --checksum`; the repository's `bindings.yaml` validates clean and `format --check` exits 0.
6. From slice 1 on, a commit that touches `src/`, `tests/`, or `tools/` and also changes a **published concern doc** (`docs/*.md` other than `README.md` and `IMPLEMENTATION.md`) fails a fitness check run over the commit range since the previous CI run (in CI the `before..sha` range GitHub supplies, passed to the test as an argument, with the checkout at `fetch-depth: 0`; when `before` is the all-zero SHA of a first push or new branch, and locally, the range is `HEAD~1..HEAD`), unless the doc change is a version bump produced by the doc-led flow, mechanically: the doc diff touches only front-matter `version:` and `status:` lines — code never leads the docs. `docs/IMPLEMENTATION.md` and `docs/README.md` are records, not published concern docs, and are exempt.
