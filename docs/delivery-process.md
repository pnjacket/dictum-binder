...
3. **The build-status row is updated** — an explicit exit criterion, not a by-product: slice built, Verified at `merge`, proof named, *Completes* column filled **as an explicit list of IDs** (never a phrase such as "all `ENTITY-*`"), because the fitness test for acceptance 3 parses it. **All six rows' *Completes* lists are pre-filled at the record's creation in slice 1** (from the playbook), so the union check holds from day one; only Built / Verified / Proof change as slices land. A slice that changes the schema rule table regenerates `lspd.schema.json` and the README checksum line in the same commit, or gate 5 is red.
...
3. **Release slice.** An LLM writes `bindings.yaml` for this repository in canonical style without invoking `lspd`, mapping every `COMPONENT-*`, `CLI-*`, `ENTITY-*`, `INV-*` to `src/lspd/` symbols and tests. It then runs `lspd validate`, fixes any error-level findings **by hand** (writes are refused while they stand), runs `lspd format` to conform the layout, confirms `lspd validate` and `lspd format --check` both exit 0, and ticks the release-gate checklist in the record. `pyproject.toml` says `1.0.0`; `git tag v1.0.0`.
