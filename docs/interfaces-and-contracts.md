...
| `CLI-COMMENT-SET` | `lspd comment set <anchor> --text TEXT` | anchor, `--text` (non-empty; may contain newlines → multi-line block; no line may carry trailing whitespace) | `OUT-COMMENT` | `ERR-NOT-FOUND` (anchor's target absent); `ERR-USAGE` (empty text, or trailing whitespace on any line) | Reads; writes. Replaces any existing comment at the anchor (a two-carrier anchor is an error-level finding, so the write is refused until the file is fixed by hand) | `CAP-COMMENT` |
