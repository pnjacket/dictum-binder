...
> One-line: the `dbind` command surface — nineteen elements, their typed inputs with empty-value semantics, one JSON envelope with fixed projections, a total error catalog, and a compatibility promise — precise enough that an LLM agent drives it without ever opening `bindings.yaml`.
...
- **`--help`** at every level (`dbind --help`, `dbind <command> --help`, `dbind <command> <subcommand> --help`) lists every argument with its optionality and one-line meaning, the command's exit codes, and its error codes. `--help` on an unknown command is `ERR-USAGE`. A bare `dbind`, an unknown command, an unknown option, or a group without its subcommand (`dbind comment`) is `ERR-USAGE` whose `details.usage` carries the help text of the deepest level reached; the envelope's `command` is then the partial path parsed so far (`""` for bare `dbind`, `"comment"` for `dbind comment`). Anchors (`header`, `binding`, …) and the `add|remove` / `set|unset` choices are positional arguments, never a fourth parser level.
...
- dictum-binder follows semantic versioning. The tool version appears in every envelope (`dbind.version`) and in `--version`; it is `1.0.0.dev0` until the release slice stamps `1.0.0`, and `dbind.schema_version` is `1` throughout.
...
- **A Dictum template change is a major** (`ADR-MAJOR-PER-TEMPLATE`); `schema_version` in the file and `dbind.schema_version` in the envelope both track it.
- The raw output of `schema` is exactly the shipped `dbind.schema.json` (`SUCCESS-SCHEMA-MATCH`).
...
1. **`get` with two IDs.** (The envelope is one compact line; wrapped here for readability.)
   `dbind get ENTITY-USER INV-USER-EMAIL-UNIQUE` →
   ```json
   {"dbind":{"version":"1.0.0","schema_version":1},"ok":true,"command":"get",
...
2. **Rejected `add-locator`.** `dbind add-locator ENTITY-X --path "src/x.py:41" --symbol f` → `ok:false`, `error.code:"ERR-INPUT-INVALID"`, `error.details.findings:[{"code":"INV-NO-LINE-NUMBERS", …}]`, exit 1, file unchanged.
3. **Missing file.** `dbind validate` in a directory with no map → `error.code:"ERR-FILE-MISSING"`, `error.message` names the path and says to run `init`, exit 2.
...
5. **Comment round trip.** `dbind comment set locator ENTITY-USER --path src/models/user.py --symbol User --text "ORM model"`; then `dbind comment get locator ENTITY-USER --path src/models/user.py --symbol User` → `result:{"anchor":{…},"text":"ORM model"}`.
6. **A refused write.** The file has a `lines:` key on some other binding. `dbind add-locator ENTITY-Y --path src/y.py --symbol g` → `ok:false`, `error.code:"ERR-FILE-INVALID"`, `error.details.findings` naming `INV-CLOSED-KEYS` and `INV-NO-LINE-NUMBERS` at their anchors, exit 1, file untouched.
7. **Raw schema.** `dbind schema > dbind.schema.json` writes the exact shipped file; `dbind schema --checksum` prints one line, the 64-hex SHA-256.
...
| `OUT-SCHEMA` | Raw: the JSON Schema document (draft 2020-12) of `ENTITY-MAP`, serialised with sorted keys, two-space indent, `ensure_ascii=false`, one trailing newline — byte-identical to the shipped `dbind.schema.json`. With `--checksum`: one line, lowercase 64-hex SHA-256 of those bytes, newline |
...
| `ERR-PARSE` | The bytes are not a YAML document the Loader can turn into a Model: syntax error, duplicate key, not a mapping at top level, non-UTF-8, BOM or CRLF (the unloadable half of `INV-BYTES`), a comment where no anchor exists (the unloadable half of `INV-COMMENT-ANCHORED`). Fixability drives the code: none of these can be fixed through dictum-binder | 2 | one fixture per condition |
