...
Owns the component decomposition, the data flow between components, the cross-cutting patterns every command follows, the technology choices, and the ADR register. The product is small in code and single-process; what needs architecture is the strict separation between the **library-facing edges** (the only places ruamel.yaml and jsonschema are touched), the **model and rules** (the canonical style), and the **command surface** (what LLMs call), so that library behaviour can never leak into the file's bytes or the envelope.
...
| Schema | one Python source; JSON file generated from it | `ADR-SCHEMA-SINGLE-SOURCE` |
| Shape validation | `jsonschema` (MIT) at runtime over the same object | `ADR-JSONSCHEMA-RUNTIME` |
| Tests | pytest | (Quality) |
| Packaging | `pyproject.toml`, `setuptools`, console script | (Delivery) |
| Runtime dependencies | ruamel.yaml, jsonschema — both MIT | (Integrations `DEP-*`) |
...
| `COMPONENT-VALIDATOR` | Produces `ENTITY-FINDING`s from a Model or from a candidate input value: first the **shape** pass through `jsonschema` over `COMPONENT-SCHEMA`'s object (mapped to `INV-CLOSED-KEYS`, `INV-ID-GRAMMAR`, `INV-ROLE-VALUES`, `INV-PATH-FORM`, … wherever the schema can express the rule), then the **rule** pass in plain Python for every remaining `INV-*`. Reports **all** findings in one run, never the first only. Optionally the path check (`CAP-PATHCHECK`) | `validate(model) -> list[Finding]`; `validate_input(value, kind) -> list[Finding]` | `COMPONENT-SCHEMA`, `COMPONENT-MODEL`; `jsonschema` (the only importer) |
...
| `COMPONENT-SCHEMA` | The JSON Schema of `ENTITY-MAP` as one Python object (the single source, `ADR-SCHEMA-SINGLE-SOURCE`); serialises it deterministically (sorted keys, fixed separators, trailing newline) and computes its SHA-256. The repository file `lspd.schema.json` is generated from it; a CI test asserts equality (`SUCCESS-SCHEMA-MATCH`) | `schema() -> dict`; `schema_json() -> bytes`; `checksum() -> str` | standard library only |
...
| `PATTERN-ATOMIC-REPLACE` | The Emitter writes to a temporary file in the target's directory (`<name>.<random>.tmp`), copies the target's permission bits, flushes and fsyncs, then `rename`s over the target. On any failure the temporary file is removed and the target is untouched (`INV-ATOMIC-WRITE`). Nothing else on disk is ever written |
...
| `ADR-JSONSCHEMA-RUNTIME` | **`jsonschema` validates the shape at runtime.** Context: the published schema and the validator must not disagree on shape. Decision: run the same object through `jsonschema`; keep cross-field rules in Python. Consequences: one more MIT dependency; shape findings are mapped from jsonschema errors to `INV-*` codes. Status: accepted |
...
1. **Import fitness**: a test asserts that `ruamel` is imported only in `loader.py` and `jsonschema` only in `validator.py` (grep over `src/lspd/`).
