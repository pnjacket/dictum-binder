...
7. **Pipeline direction**: a test asserts the module import graph is acyclic, that `model.py`, `schema.py`, `validator.py`, and `errors.py` are shared leaves any component may import, and that among the flow components (`loader` → `commands` → `emitter` → `render` → `cli`) none imports one to its right.
