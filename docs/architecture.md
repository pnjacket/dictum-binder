...
Eight components, minted in Contracts. In one sentence each: **CLI** parses arguments and dispatches; **Loader** turns file bytes into the Model, comments included; **Model** is the plain in-memory form of Domain's entities; **Validator** produces findings from a Model; **Commands** implement the twelve capabilities against the Model; **Emitter** turns a Model into canonical bytes; **Renderer** turns a result or error into the JSON envelope or the human rendering; **Schema** is the single Python source of the JSON Schema, consumed by Validator and by `dbind schema`.

Package layout: a `src/dbind/` package with one module per component (`cli.py`, `loader.py`, `model.py`, `validator.py`, `commands/` one module per capability, `emitter.py`, `render.py`, `schema.py`, plus `errors.py` — the `ERR-*` exception classes, a leaf every component may import); console script `dbind = dbind.cli:main`; tests under `tests/`; the generated schema file at the repository root as `dbind.schema.json`. The build backend is `setuptools` with `pyproject.toml` — a routine choice recorded here so it is not re-decided.

### Component interactions / data flow

Every command is one pass through a fixed pipeline. **`COMPONENT-CLI` alone orchestrates it**: for each dispatched element it calls the Loader, the Validator (pre), the Commands function, the Validator (post), the Emitter, and the Renderer in that order. The four flow components (`loader`, `commands`, `emitter`, `render`) never import one another; the shared leaves (`model`, `schema`, `validator`, `errors`) may be imported by any component.
...
- The Validator runs twice on a write: **pre** on the loaded Model (any error-level finding refuses the write with `ERR-FILE-INVALID`; warnings are reported and the write proceeds) and **post** on the mutated Model (an error here is a tool bug and aborts before the Emitter with `ERR-INTERNAL`; see `PATTERN-VALIDATE-AROUND-WRITE`).
...
3. **Shape-breaking input.** `set` receives JSON with `lines: 12`. Commands hands the candidate binding to the Validator's input check before mutating; `INV-CLOSED-KEYS` and `INV-NO-LINE-NUMBERS` both fail; nothing is mutated or written; exit 1 with the finding.
...
Concern-local decisions: the package layout and build backend above; the orchestration rule (only `cli.py` runs the pipeline; flow components never import one another); the Model as plain dataclasses carrying `ENTITY-COMMENT` values rather than library comment tokens. Cross-cutting decisions are the ADRs.
...
|---|---|---|---|
| `COMPONENT-CLI` | Entry point (the package version, read via `importlib.metadata` for the distribution `dictum-binder`, is what `--version` and the envelope report; `argparse` is built with `prog="dbind"` and a fixed 100-column formatter). Parses argv with `argparse` subparsers, resolves `--file`, `--human`, `--check-paths`, `--no-size-limit`, `--debug`, dispatches to one Commands function, holds the **single catch-all** that maps every exception to `PATTERN-ERROR-ENVELOPE`, sets the process exit code per `PATTERN-EXIT-CODES`, and **runs the pipeline for each dispatched element** (`_Run._load` then the element's method for reads: load → pre-validate → command → render; `_Run._write` for writes: load → pre-validate → target lookup → input validation → command → post-validate → emit → render, in Interfaces' error precedence), so no other component ever calls another flow component | The dictum-binder executable and every `CLI-*` element (Interfaces); `--help` at every level (`CAP-HELP`) | `COMPONENT-LOADER`, `COMPONENT-VALIDATOR`, `COMPONENT-COMMANDS`, `COMPONENT-EMITTER`, `COMPONENT-RENDERER`, `COMPONENT-SCHEMA` (for `schema`); `errors.py`; `argparse` |
| `COMPONENT-LOADER` | Resolves the target to its final real path (`SEC-SYMLINK-FINAL-TARGET`), enforces the 10 MiB size cap unless `--no-size-limit` (`SEC-FAIL-CLOSED`), reads the bytes, checks `INV-BYTES`, parses with ruamel.yaml's round-trip loader (**imported lazily inside `load()`**, so `schema`, `--help`, and `--version` never import it), and converts the result into a `COMPONENT-MODEL` value including every comment mapped to its anchor (`INV-COMMENT-ANCHORED` is checked here, since anchors exist only at this edge). Comments are harvested from the YAML **scanner's token stream** (with line marks), never from the composed tree's comment attachments, which drop comments inside multi-line flow entries and before `---`; they are the tokens the parser reports (so `#` inside a quoted scalar is not a comment), with their line positions; anchors are derived from **line adjacency in the source bytes**, never from ruamel's own comment attachment, which places comments differently. Any YAML the Model can represent — block or flow style, `---`, aliases, tags — is loaded; only unparseable or unanchorable input is a parse error. Content the Model cannot represent (an unknown key, an all-digit ID) is detected through the Model's own field and value checks (no import of the schema checker) and is dropped from the Model and reported as a finding; because writes refuse a map with error-level findings, nothing dropped can be persisted. Raises a parse error for unparseable input, a duplicate key, or a comment with no anchor | `load(path, *, size_limit=True) -> tuple[Model, list[Finding]]` | ruamel.yaml (the only importer); `COMPONENT-MODEL`; `errors.py` |
...
| `COMPONENT-COMMANDS` | One module per capability (`init`, `validate`, `format`, `get`, `list`, `set`, `add`, `remove`, `coverage`, `comment`). Each takes a Model (or none for `init`) and arguments, applies the capability's lifecycle rules from Product's `CAP-*` rows, and returns a result value plus, for writes, the mutated Model. Enforces `INV-ORDER-PRESERVED`; only `format` reorders | One function per `CLI-*` element | `COMPONENT-MODEL`, `COMPONENT-VALIDATOR` (for input checks), `errors.py` |
...
| `COMPONENT-RENDERER` | Turns a command result, findings, and errors into the JSON envelope (`OUT-*`, Interfaces) or, with `--human`, the readable rendering; prints to stdout. Never sees library objects | `render(command, result, pre, post, err, *, version, schema_version, human) -> str` — one function; an error envelope is the same call with `err` set | `COMPONENT-MODEL`, `errors.py` |
| `COMPONENT-SCHEMA` | A Python **rule table** describing `ENTITY-MAP`'s shape — every mapping's closed key set, each key's type, optionality, enum, and pattern — from which two things are generated: the JSON Schema document (draft 2020-12; serialised deterministically with sorted keys, fixed separators, trailing newline; its SHA-256 computed) and the shape checker `COMPONENT-VALIDATOR` runs (`ADR-SCHEMA-SINGLE-SOURCE`, `ADR-OWN-SHAPE-VALIDATOR`). Holds the constant `SCHEMA_VERSION = 1`, which `init` writes and `INV-SCHEMA-VERSION` compares against; a fitness test asserts it equals the major of the package version (`1.0.0.dev0` until the release). The repository file `dbind.schema.json` is generated from it; a CI test asserts equality (`SUCCESS-SCHEMA-MATCH`) | `schema() -> dict`; `schema_json() -> bytes`; `checksum() -> str`; `check_shape(value, node) -> list[Finding]`; `SCHEMA_VERSION` | `COMPONENT-MODEL` (for `Finding`); otherwise standard library only |

### Cross-cutting patterns

| ID | Pattern |
|---|---|
| `PATTERN-ERROR-ENVELOPE` | **Total.** Every failure from every source — argument parsing (argparse's own errors are intercepted, never its default stderr text), file I/O, ruamel parse errors, shape-check failures, command-level errors, and any unexpected exception — is caught at `COMPONENT-CLI`'s single catch-all and rendered as the error envelope with a stable `ERR-*` code (Interfaces). A Python traceback never reaches stdout or stderr unless `--debug` is set, and then only **in addition** to the envelope, on stderr. **Content-negotiated** in the CLI sense: the envelope is JSON by default and the human rendering under `--human`; both carry the same code and message. `KeyboardInterrupt` is the one exception: exit 130, no envelope |
| `PATTERN-VALIDATE-AROUND-WRITE` | Every write command runs the Validator **before** (on the loaded Model) and **after** (on the mutated Model). **Pre**: any error-level finding refuses the write — `ERR-FILE-INVALID`, exit 1, `details.findings` carrying them, nothing written; warnings are tolerated and reported. Candidate **input** is validated with `validate_input` before mutation; any error there aborts with `ERR-INPUT-INVALID`, nothing written. **Post**: a valid model plus valid input must yield a valid model, so a post-validation error is a tool bug by construction and aborts before the Emitter with `ERR-INTERNAL`; post warnings are reported. Both passes' findings are returned in the envelope, labelled `pre` and `post`. A `schema_version` mismatch is refused earlier still, by every command except `validate`, with `ERR-SCHEMA-VERSION` |
| `PATTERN-ATOMIC-REPLACE` | The Emitter writes to a temporary file in the **resolved** target's directory (`<name>.<random>.tmp`), copies the target's permission bits (or, when creating via `init`, sets the mode explicitly to `0o666 & ~umask`, since a secure temporary file is created `0600`), flushes and fsyncs, then `rename`s over the target. On any failure the temporary file is removed and the target is untouched (`INV-ATOMIC-WRITE`). Nothing else on disk is ever written |
| `PATTERN-EXIT-CODES` | `0` clean or warnings only · `1` the caller can fix it (validation errors, unknown ID or entry, duplicate, rejected input, usage error) · `2` the environment must change (file missing → `init`, unreadable, unparseable, unwritable, internal error) · `130` interrupted. The code is decided by `COMPONENT-CLI` from the `ERR-*` class or the highest finding severity, with one contracted exception: `format --check` exits 1 when the file would change (`OUT-FORMAT-RESULT.changed`), a caller-fixable condition that is neither an error nor a finding. No other command sets the code directly |
| `PATTERN-OUTPUT-MODE` | Default JSON on stdout, one document per invocation, no other stdout output ever, written as UTF-8 bytes to the binary stdout stream regardless of locale. `--human` switches the Renderer to the readable form. `--help` is plain text regardless of mode. stderr is used only by `--debug`. The envelope's payload for `get`/`list` is exactly the selected entries (`SUCCESS-BOUNDED-OUTPUT`) |
...
| `ADR-STRUCTURAL-ONLY` | **Validation reads nothing but the target file.** Context: the manifest, docs, and code have no fixed style; a deterministic reader of them is not buildable. Decision: doc-end and code-end checks stay with Dictum's agents; the opt-in path check is the one exception and only stats paths. Consequences: coverage gaps and unknown IDs are invisible to dictum-binder by design. Status: accepted |
| `ADR-NO-LINE-NUMBERS` | **No line references in any form.** Context: a stale line number fails silently. Decision: `lines`/`line` keys and `:NNN` suffixes are errors; the tool never emits them. Consequences: some wild maps fail validation until converted. The rule governs locators and file content; a diagnostic *message* (e.g. an `INV-BYTES` finding) may cite a line number, since it is transient output, not a stored reference. Status: accepted |
...
| `ADR-MAJOR-PER-TEMPLATE` | **A template change is a new dictum-binder major; `schema_version` tracks it.** Context: one binary should support one shape. Decision: no version flag; the file declares its shape; mismatch is an error; no migration in v1. Consequences: a Dictum template change forces a coordinated bump and, later, a migration command. Status: accepted |
...
1. **Import fitness**: a test asserts that `ruamel` is imported only in `loader.py` and that no other third-party package is imported anywhere in `src/dbind/` (grep over the tree; the standard library is the only other source).
...
6. **Schema single source**: CI regenerates `dbind.schema.json` from `COMPONENT-SCHEMA` and fails on any diff; `dbind schema --checksum` equals the SHA-256 of the file.
7. **Orchestration**: a test asserts the module import graph is acyclic; that `model.py`, `schema.py`, `validator.py`, and `errors.py` are shared leaves any component may import; that no flow component (`loader.py`, `commands/*`, `emitter.py`, `render.py`) imports another flow component; and that `cli.py` is the only module importing all four.
