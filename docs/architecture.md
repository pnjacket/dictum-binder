...
| `COMPONENT-RENDERER` | Turns a command result, findings, and errors into the JSON envelope (`OUT-*`, Interfaces) or, with `--human`, the readable rendering; prints to stdout. Never sees library objects | `render(result, findings, mode) -> str`; `render_error(err, mode) -> str` | `COMPONENT-MODEL`, `errors.py` |
