...
| `PATTERN-ATOMIC-REPLACE` | The Emitter writes to a temporary file in the **resolved** target's directory (`<name>.<random>.tmp`), copies the target's permission bits (or, when creating via `init`, leaves the umask default), flushes and fsyncs, then `rename`s over the target. On any failure the temporary file is removed and the target is untouched (`INV-ATOMIC-WRITE`). Nothing else on disk is ever written |
