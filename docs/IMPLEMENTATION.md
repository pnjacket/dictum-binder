...
Built and verified at `merge`: `CAP-INIT`, `CAP-VALIDATE`, `CAP-PATHCHECK`, `CAP-SCHEMA`, `CAP-HELP` (slice 1); `CAP-QUERY` (slice 2); `CAP-SET`, `CAP-ADD`, `CAP-REMOVE`, `CAP-COVERAGE` (slice 3). Not yet built: `CAP-COMMENT` (slice 4), `CAP-FORMAT` (slice 5) — all inside the build-ready gate scope.
