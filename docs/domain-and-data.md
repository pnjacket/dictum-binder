...
- **Comment carriers**: a header, binding, coverage, or curated comment is a block of `# ` lines **contiguous with** (no blank line between) its anchor line. A locator, field, or assertion comment is either trailing on the flow line or a block contiguous above it — both accepted on read; a blank line between a comment block and the next line breaks attachment, so such a block is unattachable (`ERR-PARSE`); on write a single-line comment is emitted trailing, a multi-line one above. Two carriers on one anchor is an error-level finding (`INV-COMMENT-ANCHORED`); a comment where no anchor exists makes the file unloadable (`ERR-PARSE`). Comment text is stored without the `# ` leader; an empty line inside a multi-line comment is stored as an empty line and emitted as a bare `#`; text never carries trailing whitespace (`CLI-COMMENT-SET` rejects it). On read, a comment line written `#text` with no space after `#` is accepted as the text `text` and re-emitted as `# text` — the one **deliberate mild default** in the model (operator's call, 2026-09-17), because YAML itself treats both forms as the same comment.
...
| `INV-COMMENT-TEXT` | Every comment's text is non-empty and no line of it carries trailing whitespace, whatever channel supplied it: a `--text`/`--comment` flag (rejected as `ERR-USAGE`, an argument rule), a comment string in `set --json` input (rejected as `ERR-INPUT-INVALID` carrying this code), or a file line (a bare `#` as a whole single-line comment — a block consisting only of bare `#` lines counts as empty — or a `# ` line with trailing whitespace, which also fires `INV-BYTES`: **both** findings are reported) | write-gated; checked on every read |
