...
- **Top-level order**: header comment, `schema_version`, `bindings`, `coverage`. `coverage` is omitted when it would be empty. `bindings` is `{}` when empty. **Blank lines**: exactly one after the `schema_version` line, one after `bindings:` when it has entries, one between bindings, one before `coverage:`; none after `bindings: {}`; the file ends with exactly one LF and no blank line. The canonical empty map is therefore the bytes `schema_version: 1\n\nbindings: {}\n`.
...
| `ENTITY-PATH` | Value type. A repository-relative path: one or more segments joined by single `/`; each segment non-empty and neither `.` nor `..`; no leading `/`; no `\`; no drive-letter prefix; no `~`; not whitespace-only; no trailing `:digits` (that last case is reported under `INV-NO-LINE-NUMBERS`, never under `INV-PATH-FORM`) |
...
| `INV-SYMBOL-NONEMPTY` | Every present `symbol` is a non-empty string; every `run`, `compare_via`, `owed`, `arm`, wire value, and curated reason likewise — and every such scalar is **single-line with no control characters** (no `\n`, `\r`, `\t`, or other C0 code point), so a value carrying a newline from the shell is rejected (`ERR-USAGE` from a flag, `ERR-INPUT-INVALID` carrying this code from JSON) rather than escaped into the file | write-gated; checked on every read |
...
| `INV-COMMENT-TEXT` | Every comment's text is **non-empty** — a bare `#` as a whole comment, a block consisting only of bare `#` lines, or a block whose first or last line is empty all count as empty — whatever channel supplied it: a `--text`/`--comment` flag (rejected as `ERR-USAGE`, an argument rule), a comment string in `set --json` input (rejected as `ERR-INPUT-INVALID` carrying this code), or a file. **Error** severity for emptiness. A comment line with trailing whitespace is the **advisory** arm of this invariant (a warning, also reported under `INV-BYTES`; `format` repairs it) | write-gated for emptiness; advisory for trailing whitespace; checked on every read |
...
Each invariant's finding is anchored as follows (`OUT-ANCHOR` in Interfaces is the projection; unmentioned keys are `null`). A `file` anchor carries no `path` (the target's path is not an `ENTITY-PATH`); a `field` anchor carries `id` and `field` only.

| Finding code | Anchor |
|---|---|
| `INV-ID-GRAMMAR` | `binding` with `id` = the offending key string |
| `INV-ID-UNIQUE` | n/a — `ERR-PARSE` |
| `INV-SCHEMA-VERSION` | `file` |
| `INV-CLOSED-KEYS`, `INV-NO-LINE-NUMBERS`, `INV-SYMBOL-NONEMPTY` | the innermost anchor that exists: `locator`/`field`/`assertion` for a key or value inside an entry, `binding` for a key or value directly under the binding (incl. anything under `wire`), `coverage`/`curated(K)` inside the coverage block, `file` at the top level |
| `INV-PATH-FORM`, `INV-PATH-EXISTS` | the `locator` or `field` carrying the path |
| `INV-ROLE-VALUES` | the `locator` |
| `INV-WIRE-SUBSET` | `binding` |
| `INV-ASSERTION-SHAPE` | the `assertion`, with whichever of `path`/`symbol`/`arm`/`owed` are present and `null` for the rest |
| `INV-FIELD-NAME` | `field` with the offending name (`""` when empty); `binding` when `fields` itself is empty |
| `INV-COVERAGE-WELLFORMED` | `curated(K)` for a per-entry rule (empty reason, kind in both lists — reported once, at the curated entry); `coverage` for every other rule |
| `INV-LOCATOR-UNIQUE`, `INV-ASSERTION-UNIQUE` | one finding per duplicate occurrence after the first, at that occurrence's anchor |
| `INV-COMMENT-ANCHORED`, `INV-COMMENT-TEXT` | the anchor the comment belongs to |
| `INV-ROLE-REQUIRES-WIRE` | `binding` |
| `INV-OWNED-TWICE` | one finding per participating binding, at its `locator`, the message naming every other binding sharing the pair |
| `INV-BYTES` | `file`, the message citing the line number (allowed by `ADR-NO-LINE-NUMBERS` for diagnostics) |
| `INV-CANONICAL-FIXPOINT`, `INV-ORDER-PRESERVED`, `INV-ATOMIC-WRITE` | tool properties; never findings |
