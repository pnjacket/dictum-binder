...
| `INV-SYMBOL-NONEMPTY` | Every present `symbol` is a non-empty string; every `run`, `compare_via`, `owed`, `arm`, wire value, and curated reason likewise — and every such scalar is **single-line with no control characters** (no `\n`, `\r`, `\t`, or other C0 code point), so a value carrying a newline from the shell is rejected (`ERR-INPUT-INVALID` carrying this code, from a flag value or from JSON alike — an *empty* flag value is the argument rule `ERR-USAGE`) rather than escaped into the file | write-gated; checked on every read |
...
| `file`, the message citing the line number (allowed by `ADR-NO-LINE-NUMBERS` for diagnostics) | `INV-BYTES` |
