...
| `INV-COMMENT-TEXT` | Every comment's text is non-empty and no line of it carries trailing whitespace, whatever channel supplied it: a `--text`/`--comment` flag (rejected as `ERR-USAGE`, an argument rule), a comment string in `set --json` input (rejected as `ERR-INPUT-INVALID` carrying this code), or a file line (a bare `#` as a whole single-line comment, or a `# ` line with trailing whitespace) | write-gated; checked on every read |
