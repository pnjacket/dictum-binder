...
- **Writes refuse a map that has errors; warnings are tolerated.** Every write validates the file first; any error-level finding refuses the write with `ERR-FILE-INVALID` and nothing is changed (the LLM fixes the file through `validate`'s findings first). Warning-level findings are reported and the write proceeds. Decided 2026-09-17 after the implementation-planner showed that tolerating error-level content would force the model to carry unrepresentable data.
