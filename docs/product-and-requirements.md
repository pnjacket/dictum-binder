...
Twelve capabilities, all **in scope for v1** (the operator marked nothing out), minted in Contracts (`CAP-INIT` through `CAP-HELP`). Cross-cutting behaviour of every command (every write — `set`, `add-*`, `remove`, `coverage`, `comment set|unset`, `format` — is refused when the file carries error-level findings, `ERR-FILE-INVALID`):
...
This concern is the root of the ID web and consumes nothing minted elsewhere. What each downstream concern mints for it (all now Contract-grade; the prefixes are pattern references):
...
| `CAP-VALIDATE` | Validate | Structural validation of the file alone; findings with severity error or warning and a stable code (the violated `INV-*`) | agent, human | in (v1) | `SUCCESS-COMPLETE-OPS`, `SUCCESS-CROSS-MODEL` | Runs on request and automatically before and after every write. Errors (a summary; Domain's `INV-*` register is the full list): a missing required key or wrong type, ID grammar, numeric ID, unknown key, missing `schema_version` or mismatch, empty `path`/`symbol`, line number anywhere, dotted member suffix in a key, missing `run` on a non-owed assertion, a kind in both `fully_bound` and `curated`, two comment carriers on one anchor, and — only with the path check on — a missing path. Warnings: `role` without `wire`; one locator realising two IDs; trailing whitespace or an EOF-newline deviation (layout-class, repaired by `format`). Excluded kinds pass silently. Unparseable or empty file: exit 2. A `schema_version` mismatch is the one finding that also stops every other command (`ERR-SCHEMA-VERSION`) |
| `CAP-FORMAT` | Format | Rewrite to canonical layout and order: bindings sorted by ID, keys in template order, flow-style entries; comments kept at their anchors | agent, human | in (v1) | `SUCCESS-ROUNDTRIP` | The only command that reorders. Refuses a map with error-level findings (`ERR-FILE-INVALID`, exit 1) — formatting what it can would silently drop the rest; warnings are kept and reported. `--check` never writes and exits 1 iff the file would change. Idempotent: a second run is a byte-identical no-op. Unparseable file: exit 2 |
...
| `CAP-SET` | Upsert a binding | Replace or create a whole binding from structured input; the stub form `locators: []` is legal | agent | in (v1) | `SUCCESS-COMPLETE-OPS` | Unknown ID creates; known ID replaces in place (order kept). Refused when the file has error-level findings (`ERR-FILE-INVALID`). Input that breaks the shape is rejected, nothing written (exit 1); warning-only input is written and reported. A numeric ID is rejected |
