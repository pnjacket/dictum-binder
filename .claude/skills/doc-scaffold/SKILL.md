...
2. **Interview to saturation (interactive default).** Ground in the brief, then **keep interviewing in small rounds** (a handful of questions each) until a round surfaces nothing new or the operator calls it — do NOT use a fixed questionnaire, and do NOT stop because you've "inferred enough" (that minimal posture is the autonomous-mode concession, not the interactive default — STANDARD Part 0.6). Adaptive means *which* questions are in scope, never *how few*. Drive the rounds from a **coverage map**: the traits below × each in-scope concern's sub-aspects × each capability's lifecycle/edge/failure/NFR dimensions; each round targets the still-uncovered cells and you tell the operator what's covered vs still open. The traits that drive scope:
   ...
   - **When the operator can't answer (the honest exit, so no one gets stuck).** On every question, make explicit that *"I'm not able to — or not the right person to — answer this"* is a valid response; never press the operator toward a guess. When they take it: record the item as a **`[GAP]`** (an honest product hole), **never** as an `[ASSUMPTION]` (that is inference, which interactive mode forbids); **do not re-ask that same operator the same question this session** — that loop is the failure this prevents — and carry it forward as a visible, gate-blocking `[GAP]` a later round or a more capable operator can fill. If instead they say *"I can answer, but not until X is decided,"* **defer, don't force**: reorder so X comes first and re-surface this question once X is settled; if the session ends before X, it carries as an open `[GAP]` re-asked next round. A `[GAP]` that stays unanswerable across operators is eventually **adjudicated** — answered by a capable operator, or scoped out (Part 9) — never inferred.
...
6. **Write the manifest** from `templates/manifest.template.md`: traits, packaging, per-concern `in_scope` + `current_rung` + `target_rung` (default contract-grade) + `out_of_scope_subaspects`, and **`authored_against:` = the installed standard's signed release tag** — stamp it at creation so the set is born with the provenance every later upgrade walk needs as its start line (or `<base-tag>+g<short-sha>` when the installed checkout is on an un-tagged commit; never stamp a bare front-matter `version:` as if it were a signed tag). Carry a `standard_source` into the manifest if the install recorded one.
...
- **Never press an operator toward a guess.** The "can't / not-my-place to answer" exit is always on offer; taking it yields a `[GAP]` (not an `[ASSUMPTION]`), and the question is carried forward — not re-asked of the same stuck operator this session.
